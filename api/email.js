...
module.exports = async function handler(req, res) {
  ...
  if (action === 'send-reminder') {
    const { recipientEmail, fromName, fromEmail, message, invoiceNumber, amount, totalAmt, paidAmt, txType, isSelf, isReminder, entryStatus, currency, viewUrl, siteUrl, appName, tagline, logoData } = req.body;
    if (!recipientEmail) return res.status(400).json({ ok: false, error: 'recipientEmail is required.' });
    const result = await sendReminderEmail({ to: recipientEmail, fromName, fromEmail, message, invoiceNumber, amount, totalAmt, paidAmt, txType, isSelf, isReminder, entryStatus, currency, viewUrl, siteUrl, appName, tagline, logoData });
