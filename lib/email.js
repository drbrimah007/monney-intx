...
async function sendReminderEmail({ to, fromName, fromEmail, message, invoiceNumber, amount, totalAmt, paidAmt, viewUrl, siteUrl, appName, tagline, txType, entryStatus, logoData }) {
  ...
  if (amount != null) {
    if (hasBreakdown) {
      const _total   = parseFloat(totalAmt).toFixed(2);
      const _paid    = parseFloat(paidAmt).toFixed(2);
      const _bal     = parseFloat(amount).toFixed(2);
      amtLine = `
        <div style="margin-bottom:14px;border:1px solid #e5e7eb;border-radius:8px;overflow:hidden;">
          <div style="background:#f9fafb;padding:10px 16px;border-bottom:1px solid #e5e7eb;display:flex;justify-content:space-between;">
            ...
            <span style="font-size:14px;font-weight:600;color:#555;">$${_total}</span>
          </div>
          <div style="background:#f0fdf4;padding:10px 16px;border-bottom:1px solid #e5e7eb;display:flex;justify-content:space-between;">
            <span style="font-size:12px;color:#888;text-transform:uppercase;letter-spacing:.05em;">Paid</span>
            <span style="font-size:14px;font-weight:600;color:#16a34a;">− $${_paid}</span>
          </div>
          <div style="background:#fff;padding:12px 16px;display:flex;justify-content:space-between;align-items:center;">
            ...
            <strong style="font-size:22px;color:#1a1a2e;">$${_bal}</strong>
          </div>
        </div>`;
    } else {
      amtLine = `<div style="margin-bottom:14px;"><span style="font-size:12px;color:#888;text-transform:uppercase;letter-spacing:.05em;">${amtLabel}</span><br><strong style="font-size:24px;color:#1a1a2e;">$${parseFloat(amount).toFixed(2)}</strong></div>`;
    }
  }
  ...
  return sendEmail({
    ...
    text: `${heading}\n${fromName}\n\n${message}\n\n${invoiceNumber ? 'Reference: ' + invoiceNumber + '\n' : ''}${hasBreakdown ? 'Original: $' + parseFloat(totalAmt).toFixed(2) + '\nPaid: $' + parseFloat(paidAmt).toFixed(2) + '\n' + amtLabel + ': $' + parseFloat(amount).toFixed(2) + '\n' : (amount != null ? amtLabel + ': $' + parseFloat(amount).toFixed(2) + '\n' : '')}\nView entry: ${ctaUrl}\n\n— ${siteName}`
  });
}
