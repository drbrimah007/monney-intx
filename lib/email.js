...
async function sendReminderEmail({ to, fromName, fromEmail, message, invoiceNumber, amount, viewUrl, siteUrl, appName, tagline, txType, logoData }) {
  ...
  if (txType === 'you_owe_them') {
    ...
    subheading = `<strong>${fromName}</strong> has shared a record with you.`;
    amtLabel   = 'Amount Due';
  }
  ...
  // Logo: only embed if logoData is an absolute HTTP URL (data: URIs are blocked by email clients)
  const validLogoUrl = (logoData && /^https?:\/\//i.test(logoData)) ? logoData : null;
  const logoHtml = validLogoUrl
    ? `<img src="${validLogoUrl}" alt="${siteName}" style="height:48px;max-width:180px;object-fit:contain;display:block;margin:0 auto 8px;">`
    : `<div style="font-size:22px;font-weight:900;color:#fff;letter-spacing:.02em;text-align:center;">${siteName}</div>`;
