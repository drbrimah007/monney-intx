...
async function sendReminderEmail({ to, fromName, fromEmail, message, invoiceNumber, amount, viewUrl, siteUrl, appName, tagline, txType, logoData }) {
  ...
  // Direction-aware subject + heading
  let subject, heading, subheading, amtLabel;
  if (txType === 'you_owe_them') {
    subject    = `Record from ${fromName}: they acknowledge owing you`;
    heading    = 'Balance Notice';
    subheading = `<strong>${fromName}</strong> has recorded that they owe you.`;
    amtLabel   = 'Amount owed to you';
  } else if (txType === 'you_paid_them') {
    subject    = `${fromName} has recorded a payment`;
    heading    = 'Payment Record';
    subheading = `<strong>${fromName}</strong> has recorded making a payment.`;
    amtLabel   = 'Amount recorded';
  } else if (txType === 'they_paid_you') {
    subject    = `${fromName} has recorded your settlement`;
    heading    = 'Settlement Record';
    subheading = `<strong>${fromName}</strong> has recorded that you settled a balance.`;
    amtLabel   = 'Amount settled';
  } else {
    // they_owe_you / invoice / bill — recipient owes sender
    subject    = `Payment reminder from ${fromName}`;
    heading    = 'Payment Reminder';
    subheading = `From <strong>${fromName}</strong>`;
    amtLabel   = 'Amount';
  }
  ...
  const replyLine = fromEmail ? `<p style="color:#666;font-size:13px;">To respond, contact <a href="mailto:${fromEmail}" style="color:#6c63ff;">${fromEmail}</a> directly.</p>` : '';

  // Logo: use only absolute HTTP URLs — data: URIs are blocked by most email clients
  const logoUrl = (logoData && /^https?:\/\//i.test(logoData))
    ? logoData
    : `${siteBase}/logo.svg`;
  const logoHtml = `<img src="${logoUrl}" alt="${siteName}" style="height:48px;max-width:180px;object-fit:contain;display:block;margin:0 auto 8px;" onerror="this.style.display='none'">`;
    ...
    html: `
      <div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:0;background:#f9f9f9;">
        <div style="background:#1a1a2e;padding:20px 32px 16px;border-radius:12px 12px 0 0;text-align:center;">
          ${logoHtml}
          <span style="font-size:18px;font-weight:800;color:#fff;letter-spacing:-0.3px;">${siteName}</span>
        </div>
